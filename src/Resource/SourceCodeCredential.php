...
class SourceCodeCredential
{
     ...
     * @var string
     */
    protected $resourceName = 'sourceCodeCredential/';

    /**
     * @var string
     */
    protected $resourcePluralName = 'sourceCodeCredentials/';
     ...
     *
     * @throws \Exception
     * @return \Rancher\Model\SourceCodeCredential
     */
    public function get($id)
    {
        $response = $this->client->request('GET', $this->constructPath() . $id, []);

        return $this->client->getSerializer()->deserialize($response, '\Rancher\Model\SourceCodeCredential');
    }
    ...
    {
        $this->client->request('POST', $this->constructPath() . $id . '?action=logout', []);
     ...
     * @return void
     */
    public function refreshrepos($id)
    {
        $this->client->request('POST', $this->constructPath() . $id . '?action=refreshrepos', []);

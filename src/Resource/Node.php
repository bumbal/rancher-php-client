...
class Node
{
     ...
     * @var string
     */
    protected $resourceName = 'node/';

    /**
     * @var string
     */
    protected $resourcePluralName = 'nodes/';
     ...
     *
     * @throws \Exception
     * @return \Rancher\Model\Node
     */
    public function get($id)
    {
        $response = $this->client->request('GET', $this->constructPath() . $id, []);

        return $this->client->getSerializer()->deserialize($response, '\Rancher\Model\Node');
    }

    /**
     * set
     *
     * @param \Rancher\Model\Node $data
     *
     * @throws \Exception
     * @return \Rancher\Model\Node
     */
     ...
     * @return void
     */
    public function cordon($id)
    {
        $this->client->request('POST', $this->constructPath() . $id . '?action=cordon', []);
     ...
     *
     * @param string $id
     * @param \Rancher\Model\NodeDrainInput $input
     *
     * @throws \Exception
     * @return void
     */
    public function drain($id, $input)
    {
        $postData = json_encode(\Rancher\ObjectSerializer::sanitizeForSerialization($input));

        $this->client->request('POST', $this->constructPath() . $id . '?action=drain', $postData);
     ...
     * @return void
     */
    public function stopDrain($id)
    {
        $this->client->request('POST', $this->constructPath() . $id . '?action=stopDrain', []);
     ...
     * @return void
     */
    public function uncordon($id)
    {
        $this->client->request('POST', $this->constructPath() . $id . '?action=uncordon', []);
